...
export class CreateEmployeeDto {
    @ApiProperty({description: "The department of the employee irrespective of the role."})
    @IsString()
    @IsNotEmpty()
    
    @ApiProperty({description: "The position of the employee in the organization."})
    @IsString()
    @IsNotEmpty()
    position: string
   ...
   @IsNumber({maxDecimalPlaces: 2}, {message: "Amount must be a valid decimal number."})
    ...
    @ApiProperty({description: "The status of the employee, be it ACTIVE, INACTIVE, SUSPENDED OT TERMINATED."})
    @IsEnum(Status, {message: "Status must be ACTIVE, INACTIVE, SUSPENDED OR TERMINATED."})
    status: Status
    
    @ApiProperty({description: "The ID of the manager for employees that are neither the manager or an HR."})
    @IsOptional()
    @IsUUID()
    managerId?: string

    @ApiProperty({description: "The type of contract, be it a FULL_TIME OR just on a CONTRACT."})
    @IsEnum(ContractType, {message: "Contract type must be FULL_TIME OR CONTRACT"})
    contractType: ContractType

    @ApiProperty({description: "The end date of the employee in format of YYY-MM-DD"})
    @IsOptional()
    ...
    @ApiProperty({description: "The date at which the user was employed to the company"})
    @IsOptional()
    @IsNotEmpty()
    @IsDateString({}, {message: "The date must be a valid ISO 8601 string (e.g., YYYY-MM-DD)" })
    @Transform(({ value }) => new Date(value).toISOString(), { toClassOnly: true })
